 ...
 * Counselor API.
 *
 * Ported from flymaster-counselor-portal/server/index.mjs, which ran as its own
 * Express service, against the same database, with its own JWT secret and an
 * auth middleware that verified the token but checked no role at all — any valid
 * counselor-realm token reached every counselor route.
 *
 * Here every route runs on the shared counselorAuth stack (session -> role gate
 * -> branch scope), and routes are namespaced under /api/counselor/* so they no
 * longer shadow the shared endpoints in core.mjs.
 */
...
import {
  ...
  IS_PRODUCTION,
  JWT_SECRET,
  COUNSELOR_SIGNUP_CODE,
  EMAIL_VERIFICATION_EXPIRY_MINUTES,
} from "../lib/env.mjs";
import { ROLES, signUser, session, requireRole, branchScope, audit, counselorAuth } from "../lib/auth.mjs";
...
router.post("/api/counselor/auth/send-verification-code", async (req, res) => {
  try {
...
router.post("/api/counselor/auth/signup", async (req, res) => {
  try {
    const email = String(req.body.email || "").trim().toLowerCase();
...
router.put("/api/counselor/profile", counselorAuth, async (req, res) => {
  try {
...
router.get("/api/counselor/state", counselorAuth, async (req, res) => {
...
router.post("/api/counselor/attendance", counselorAuth, async (req, res) => {
...
router.patch("/api/counselor/attendance/:id", counselorAuth, async (req, res) => {
  await pool.query(
...
router.post("/api/counselor/shortlists", counselorAuth, async (req, res) => {
  try {
...
router.patch("/api/counselor/applications/:id", counselorAuth, async (req, res) => {
  const status = String(req.body.status || "").trim();
...
router.post("/api/counselor/notifications/read", counselorAuth, async (req, res) => {
...
router.post("/api/counselor/notifications/:id/read", counselorAuth, async (req, res) => {
...
router.post("/api/counselor/messages", counselorAuth, async (req, res) => {
...
router.patch("/api/counselor/messages/:id", counselorAuth, async (req, res) => {
...
router.post("/api/counselor/conversations/:id/read", counselorAuth, async (req, res) => {
  await pool.query(
...
router.get("/api/counselor/university-catalog/universities", counselorAuth, async (req, res) => {
  try {
...
router.get("/api/counselor/university-catalog/degrees", counselorAuth, async (req, res) => {
  try {
    const country = String(req.query.country || "").trim();
...
router.get("/api/counselor/university-programs", counselorAuth, async (req, res) => {
  try {
...
router.get("/api/counselor/students/:id/checklist", counselorAuth, async (req, res) => {
  try {
...
router.post("/api/counselor/students/:id/document-requests", counselorAuth, async (req, res) => {
  try {
...
router.get("/api/counselor/checklists", counselorAuth, async (_req, res) => {
  try {
...
router.post("/api/counselor/checklists", counselorAuth, async (req, res) => {
  try {
...
router.patch("/api/counselor/checklists/:id", counselorAuth, async (req, res) => {
  try {
    const items = await jsonTable("document_checklists");
...
/*
 * The counselor service mounted a SECOND WhatsApp implementation here, on the
 * same database as the one in routes/core.mjs. Every path it registered
 * (/api/whatsapp/webhook, /send-otp, /verify-otp, /conversations, /messages,
 * /conversations/:id/read) already exists in core.mjs, so with both services
 * live the behaviour depended on which one a given portal happened to call.
 * Core's copy is now the single implementation and counselors reach it through
 * the shared staff stack.
 *
 * Three endpoints existed ONLY in this copy and are not yet ported:
 *     GET  /api/whatsapp/status
 *     POST /api/whatsapp/app-message
 *     GET  /api/whatsapp/my-thread
 * Confirm whether any portal calls them before deleting lib/whatsappRoutes.mjs.
 *
 * It also had a real privilege bug: verifyJwt returned { ...claims, role:
 * "counselor" } for ANY valid token, so a student or telecaller token reached
 * staff-only chat routes. The shared session() reads the role from the database
 * instead.
 */
