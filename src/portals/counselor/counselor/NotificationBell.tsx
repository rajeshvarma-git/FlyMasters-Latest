...
import { markNotificationRead, useLocalStore } from "@counselor/lib/store";
import type { LocalNotification } from "@counselor/lib/store";
import { notificationIcon, notificationUrl } from "@counselor/counselor/notificationUtils";
      ...
      {open && (
        <div className="absolute right-0 z-50 mt-2 w-[min(92vw,380px)] overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl">
          <div className="flex items-center justify-between border-b px-4 py-3">
            ...
            <Link to="/notifications" className="text-xs font-medium text-sky-600 hover:underline" onClick={() => setOpen(false)}>
              View all
            </Link>
          </div>
