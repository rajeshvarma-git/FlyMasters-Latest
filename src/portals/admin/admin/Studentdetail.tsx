...
// // // import { counselorLabel, displayName, initials, studentOwns, telecallerLabel } from "@admin/lib/utils";
...
// // //     return (
// // //       <div>
// // //         <Link to="/students" className="mb-4 inline-flex items-center gap-2 text-sm text-slate-600 hover:text-sky-600">
...
// //                   {counselor ? (
// //                     <Link to={`/counselors/${counselor.id}`} className="text-sky-700 hover:underline">
// //                       {displayName(counselor.first_name, counselor.last_name, counselor.email)}
// //                     </Link>
// //                   ) : (
// //                     "Not assigned"
...
export default function StudentDetail() {
  ...
  const backTo = from?.startsWith("telecaller/")
    ? `/telecallers/${from.slice("telecaller/".length)}`
    : from?.startsWith("counselor/")
      ? `/counselors/${from.slice("counselor/".length)}`
      : "/students";
  ...
  if (!isConvertedStudent(student)) {
    ...
    return <Navigate to={`/leads/${student.id}${params ? `?${params}` : ""}`} replace />;
  }
